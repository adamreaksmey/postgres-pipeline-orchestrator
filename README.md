...
Mini-Jenkin is a **lightweight CI/CD orchestrator** built with **NestJS** and **PostgreSQL 17**, designed to prove that PostgreSQL can handle *everything*: job queues, real-time logs, deployment locking, retries, and metrics—all **without Redis, Kafka, RabbitMQ, or any external message broker**.

Think of PostgreSQL not just as a database, but as the *engine of your distributed system*.
...
*Postgres is no longer just storage—it’s your queue, pub/sub system, scheduler, and analytics engine.*
...
*PostgreSQL is at the heart of the system, replacing traditional brokers and orchestration tools.*
...
Why rely on Kafka, RabbitMQ, or Redis when your database is powerful enough to be *everything*?
